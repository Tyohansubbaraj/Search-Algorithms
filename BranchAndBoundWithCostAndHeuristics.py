...
class BranchAndBoundWithCostAndHueristics:
    ...
    def algorithmBranchAndBoundWithCostAndHueristics(self,start,end,path = []):
        ...
        for node in nodes:
            ...
            if node[1] not in path:
                new_paths = self.algorithmBranchAndBoundWithCostAndHueristics(node[1],end,path)
                for p in new_paths:
                    paths.append(p)
    ...
    def showPathBranchAndBoundWithCostAndHueristics(self,start,end):

        paths = self.algorithmBranchAndBoundWithCostAndHueristics(start,end)
